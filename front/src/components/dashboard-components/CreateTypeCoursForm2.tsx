...
export function CreateTypeCoursForm2({
  newModules,
  // newVideos,
  setNewVideos,
  // newTexts,
  ...
  async function handleValuesChange(allValues: ModuleContents): Promise<void> {
    ...
    // Reconstruit l'objet allValues sans les valeurs vides (undefined)
    // const newAllValues = allValuesKeys.reduce<ModuleContents>((acc, key) => {
    //   console.log("acc", acc, "key", key);
    //   acc[key] = allValues[key]?.filter((content) => content !== undefined || []);
    //   return acc;
    // }, {});
