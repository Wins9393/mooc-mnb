...
export function ContentModule({
  ...
  useEffect(() => {
    if (user && currentFormation) {
      ...
      getProgression(user.id, currentFormation.id);
    }
  }, [userProgression]);

  useEffect(() => {
    displayModalNextContent(isFormationComplete);
  }, [progress]);

  function displayModalNextContent(isFormationComplete: Function) {
    const displayed = localStorage.getItem(`formation-${currentFormation?.id}-user-${user?.id}`);

    if (user && currentFormation) {
      if (isFormationComplete(progress) && displayed !== "true") {
        setIsNextContentOpen(true);
        localStorage.setItem(`formation-${currentFormation?.id}-user-${user?.id}`, "true");
      }
    }
  }
