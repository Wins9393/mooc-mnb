  ...
  return (
    <div className="formationItem__card">
      {levelProgression === "awarded" ? (
        <img src="/gold.png" style={{ width: "60px", position: "absolute", bottom: "-50px", right: "8px", zIndex: 2 }}></img>
      ) : levelProgression === "complete" ? (
        <img src="silver.png" style={{ width: "60px", position: "absolute", bottom: "-50px", right: "8px", zIndex: 2 }}></img>
      ) : (
        ""
      )}
      <img className="formationItem__card--imageCover" src={`${import.meta.env.VITE_API_URL}/public/${formation.cover_path}`} alt={formation.cover_path} />
      <div className="formationItem__card--overlay">
        <h2 className="formationItem__card--title">{formation.title}</h2>
        <p className="formationItem__card--info">{formation.modules.length > 1 ? `${formation.modules.length} modules` : `${formation.modules.length} module`}</p>
      </div>
      <Button type="primary" className="button formationItem__card--button" onClick={() => navigate(`/formation/${formation.id}`)}>
        Commencer
      </Button>
    </div>
  );
